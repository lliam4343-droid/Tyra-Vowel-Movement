...
function getDataUrl() {
  if (typeof DATA_CSV_URL !== "undefined" && DATA_CSV_URL) return DATA_CSV_URL;
  if (typeof SHEET_CSV_URL !== "undefined" && SHEET_CSV_URL) return SHEET_CSV_URL;
  return "";
}
...
async function main() {
  ...
  let res;
  try {
    res = await fetch(getDataUrl(), { cache: 'no-store' });
  } catch (e) {
    dbg('FETCH ERROR: ' + (e && e.message ? e.message : String(e)));
    setStatus('Fetch failed. See debug log.');
    return;
  }
  ...
  dbg('CSV first 200 chars: ' + csv.slice(0,200).replace(/\n/g,'\\n'));
